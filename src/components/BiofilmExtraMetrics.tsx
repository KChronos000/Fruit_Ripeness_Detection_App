...
const BiofilmExtraMetrics = ({ selectedState, dayCount }: { selectedState: any; dayCount: number }) => {
  return (
    <div className="lg:col-span-2 bg-[#182b21] border border-[#efead9]/10 rounded-2xl p-4 sm:p-5 space-y-4">
      {/* Detailed Analytics Panel Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start gap-2 border-b border-[#efead9]/10 pb-3">
        <div>
          <h2 className="text-base sm:text-lg font-bold font-serif text-[#efead9]">
            รายละเอียดวิเคราะห์: {selectedState.def.zone} — {selectedState.def.name}
          </h2>
          <p className="text-xs text-[#93a89a] mt-0.5">
            รหัสเซนเซอร์ #{selectedState.def.id} · เคลือบฟิล์มมาแล้ว {Math.floor(dayCount)} วัน
          </p>
        </div>
        <span className={`text-xs px-3 py-1 rounded-full font-bold border self-start sm:self-auto ${selectedState.bgClass}`}>
          {selectedState.full}
        </span>
      </div>
      ...
      <div>
        <div className="text-xs text-[#93a89a] mb-2 flex justify-between items-center">
          <span>แนวโน้มค่า pH ย้อนหลัง (การเปลี่ยนสีฟิล์ม)</span>
          <span className="font-mono text-amber-300 font-semibold">pH ปัจจุบัน {selectedState.ph.toFixed(2)}</span>
        </div>
      ...
      {/* Biofilm Extra Metrics Grid (แก้ไขจุดนี้) */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="bg-[#122119] p-3 rounded-xl border border-[#efead9]/5">
          <div className="text-[10px] text-[#6c8072] uppercase font-mono tracking-wider">
            น้ำมันหอมระเหยไล่แมลง
          </div>
        ...
        <div className="bg-[#122119] p-3 rounded-xl border border-[#efead9]/5">
          ...
          <div className="text-base sm:text-lg font-mono font-bold text-teal-300 mt-1 leading-snug">
